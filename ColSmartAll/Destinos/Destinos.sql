 ...
 */
	...
	drop table colsmart_snc_linea_base.z_x_clc4_final;
 ...
 */
drop table colsmart_snc_linea_base.z_x_clc4_predios;
 ...
 */	
	...
	drop table colsmart_snc_linea_base.z_x_clc4_predios_update;
	
	create table colsmart_snc_linea_base.z_x_clc4_predios_update as
	select f.codigo,f.destino, p.destinacion_economica
	from colsmart_snc_linea_base.z_x_clc4_final f
	left join colsmart_snc_linea_base.z_x_clc4_predios  p
 ...
 */		
UPDATE colsmart_test5_owner.ilc_predio
SET destinacion_economica = u.destino
FROM colsmart_snc_linea_base.z_x_clc4_predios_update u
WHERE CAST(ilc_predio.numero_predial_nacional AS REAL) = u.codigo;
...
UPDATE colsmart_test5_owner.ilc_predio
SET destinacion_economica = CASE
    WHEN destinacion_economica = 'Lote urbanizable no urbanizado' THEN 'Lote_Urbanizable_No_Urbanizado'
    WHEN destinacion_economica = 'Lote urbanizado no construido o edificado' THEN 'Lote_Urbanizable_No_Construido'
    WHEN destinacion_economica = 'Minero' THEN 'Mineria_Hidrocarburos'
    WHEN destinacion_economica = 'Servicios Especiales' THEN 'Servicios_Funerarios'
    ELSE destinacion_economica
END
WHERE destinacion_economica IN (
...
UPDATE colsmart_test5_owner.ilc_predio
SET destinacion_economica = 'Servicios_Funerarios'
WHERE destinacion_economica = 'Uso_Publico';
