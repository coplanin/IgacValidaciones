 ...
 */
	...
	DROP TABLE IF EXISTS colsmart_snc_linea_base.z_b_construccion_terreno;
	
	CREATE TABLE colsmart_snc_linea_base.z_b_construccion_terreno as	
	WITH cons AS (      -- construcciones (R + U) con posible terreno previo
	    /* ----- construcciones resguardo (Rurales) ----- */
	     SELECT
		    ...
		    identificador,
		    shape,
		    sde.st_area(shape)                                AS area_,
		    sde.st_pointonsurface(shape)                      AS pt,
		    numero_pisos,
		    terreno_codigo,
		    gs.piso
		    -- ← campo con el piso
	    ...
	    END AS n_pisos
		from colsmart_snc_linea_base.z_y_r_construccion_40)  c
		CROSS JOIN LATERAL generate_series(1, c.n_pisos) AS gs(piso) 
		where sde.st_geometrytype(shape)::text in ('ST_POLYGON','ST_MULTIPOLYGON')
		UNION ALL
		...
		FROM (select *,
		 case
	        ...
	        ELSE numero_pisos                                     -- nulo → “nuevo”
	    END AS n_pisos
		from colsmart_snc_linea_base.z_y_u_construccion_40)  c
		CROSS JOIN LATERAL generate_series(1, c.n_pisos) AS gs(piso) 
		where sde.st_geometrytype(shape)::text in ('ST_POLYGON','ST_MULTIPOLYGON')
	),
	...
	LEFT JOIN terrenos t
	       ON sde.st_intersects(c.pt, t.terreno_shape) and t.terreno_codigo IS  NULL ;
	
	 
	
	
	select count(*)
	from  colsmart_snc_linea_base.z_b_construccion_terreno;
 ...
 */	    
	...
	DROP TABLE IF EXISTS colsmart_snc_linea_base.z_b_construccion_casouno;

	CREATE TABLE colsmart_snc_linea_base.z_b_construccion_casouno AS
	SELECT DISTINCT ON (
	    c.codigo,
	  ...
	  u.tipo_planta
	FROM
	  colsmart_snc_linea_base.z_b_construccion_terreno AS c
	JOIN
	  colsmart_snc_linea_base.z_u_r_unidad_data    AS u
	...
	select count(*)
	from colsmart_snc_linea_base.z_b_construccion_casouno;
...
-- deduplicando individualmente por objectid y por id_caracteristicasunidadconstru
	...
	DROP TABLE IF EXISTS colsmart_snc_linea_base.z_b_construccion_casodos;
	
	CREATE TABLE colsmart_snc_linea_base.z_b_construccion_casodos AS
	WITH candidatos AS (
	  SELECT
	    c.objectid,
	    c.codigo,
	  ...
	  FROM colsmart_snc_linea_base.z_b_construccion_terreno AS c
	  JOIN colsmart_snc_linea_base.z_u_r_unidad_data    AS u
	    ON c.codigo = u.numero_predial
	   and c.piso::int=REGEXP_REPLACE(u.planta_ubicacion, '[^\d]+', '', 'g')::int
	   AND u.area_construccion
	       BETWEEN c.area_ * 0.95 AND c.area_ * 1.05
	  WHERE
	    ...
	    NOT EXISTS (
	      SELECT 1
	        FROM colsmart_snc_linea_base.z_b_construccion_casouno cas1
	...
	select count(*)
	from colsmart_snc_linea_base.z_b_construccion_casodos;
 ...
 */
	...
	DROP TABLE IF EXISTS colsmart_snc_linea_base.z_b_construccion_casotres;
	
	CREATE TABLE colsmart_snc_linea_base.z_b_construccion_casotres AS
	WITH
	  unmatched_construccion AS (
	    select 	  t.*
	    FROM colsmart_snc_linea_base.z_b_construccion_terreno t
	    WHERE
	      -- Excluir objectid de Caso 1
	      NOT EXISTS (
	        SELECT 1
	        FROM colsmart_snc_linea_base.z_b_construccion_casouno c
	        WHERE c.objectid = t.objectid
	      )
	      -- Excluir objectid de Caso 2
	      AND NOT EXISTS (
	        SELECT 1
	        FROM colsmart_snc_linea_base.z_b_construccion_casodos c
	        WHERE c.objectid = t.objectid
	      )
	  ),
	  unmatched_unidad AS (
	    SELECT
	      u.*
	    FROM colsmart_snc_linea_base.z_u_r_unidad_data u
	    WHERE
	      ...
	      AND NOT EXISTS (
	        SELECT 1
	        FROM colsmart_snc_linea_base.z_b_construccion_casodos cas2
		...
		select count(*) 
		from colsmart_snc_linea_base.z_b_construccion_casotres;
 ...
 */
	...
	DROP TABLE IF EXISTS colsmart_snc_linea_base.z_b_construccion_casocuatro;
	
	CREATE TABLE colsmart_snc_linea_base.z_b_construccion_casocuatro AS
	WITH
	  unmatched_construccion AS (
	    select t.*
	    FROM colsmart_snc_linea_base.z_b_construccion_terreno t
	    WHERE
	      ...
	      AND NOT EXISTS (
	        SELECT 1
	        FROM colsmart_snc_linea_base.z_b_construccion_casotres  c
	        WHERE c.objectid = t.objectid
	      )
	  ),
	  unmatched_unidad AS (
		...
		select count(*) 
		from colsmart_snc_linea_base.z_b_construccion_casocuatro;
 ...
 */
	...
	DROP TABLE IF EXISTS colsmart_snc_linea_base.z_b_construccion_casocinco;
	
	CREATE TABLE colsmart_snc_linea_base.z_b_construccion_casocinco AS
	WITH
	  unmatched_construccion AS (
	    select t.*
	    FROM colsmart_snc_linea_base.z_b_construccion_terreno t
	    WHERE
	      ...
	      AND NOT EXISTS (
	        SELECT 1
	        FROM colsmart_snc_linea_base.z_b_construccion_casocuatro c
	        WHERE c.objectid = t.objectid
	      )
	  ),
	  unmatched_unidad AS (
 ...
 */
	...
	DROP TABLE IF EXISTS colsmart_snc_linea_base.z_b_construccion_casoseis;
	
	CREATE TABLE colsmart_snc_linea_base.z_b_construccion_casoseis AS
	WITH
	  unmatched_construccion AS (
	    select t.*
	    FROM colsmart_snc_linea_base.z_b_construccion_terreno t
	    WHERE
	      ...
	      AND NOT EXISTS (
	        SELECT 1
	        FROM colsmart_snc_linea_base.z_b_construccion_casocinco c
	        WHERE c.objectid = t.objectid
	      )
	  ),
	  unmatched_unidad AS (
 ...
 */
	...
	DROP TABLE IF EXISTS colsmart_snc_linea_base.z_b_construccion_casosiete;
	
	CREATE TABLE colsmart_snc_linea_base.z_b_construccion_casosiete AS
WITH
  unmatched_construccion AS (
    select t.*
    FROM colsmart_snc_linea_base.z_b_construccion_terreno t
    WHERE
      ...
      AND NOT EXISTS (
        SELECT 1
        FROM colsmart_snc_linea_base.z_b_construccion_casoseis c
        WHERE c.objectid = t.objectid
      )
  ),
  unmatched_unidad AS (
 ...
 */
	...
	DROP TABLE IF EXISTS colsmart_snc_linea_base.z_b_construccion_casoocho;
	
		
	CREATE TABLE colsmart_snc_linea_base.z_b_construccion_casoocho as
WITH
  unmatched_construccion AS (
    SELECT   t.*
    FROM colsmart_snc_linea_base.z_b_construccion_terreno t
 ...
 */
	...
	DROP TABLE IF EXISTS colsmart_snc_linea_base.z_b_construccion_final;
	
	CREATE TABLE colsmart_snc_linea_base.z_b_construccion_final as
	-- Caso 1
	SELECT
	  c1.objectid,
	  c1.codigo,
	  ...
	  '1'                            AS caso
	FROM colsmart_snc_linea_base.z_b_construccion_casouno c1
	JOIN colsmart_snc_linea_base.z_b_construccion_terreno c0
	...
	SELECT
	  ...
	  '2'                            AS caso
	FROM colsmart_snc_linea_base.z_b_construccion_casodos c2
	JOIN colsmart_snc_linea_base.z_b_construccion_terreno c0
	  ON c2.objectid = c0.objectid
	JOIN colsmart_snc_linea_base.z_u_r_unidad_data u2
	...
	SELECT
	  ...
	  '3'                           AS caso
	FROM colsmart_snc_linea_base.z_b_construccion_casotres c3
	JOIN colsmart_snc_linea_base.z_b_construccion_terreno c0
	  ON c3.objectid = c0.objectid
	JOIN colsmart_snc_linea_base.z_u_r_unidad_data u3
	...
	SELECT
	  ...
	  u3.id_operacion_predio,
	  '7'                           AS caso
	FROM colsmart_snc_linea_base.z_b_construccion_casosiete c
	JOIN colsmart_snc_linea_base.z_b_construccion_terreno c0
	  ON c.objectid = c0.objectid
	JOIN colsmart_snc_linea_base.z_u_r_unidad_data u3
	  ON u3.id_caracteristicasunidadconstru = c.id_caracteristicasunidadconstru
	...
	SELECT
	  ...
	  u3.id_operacion_predio,
	  '8'                           AS caso
	FROM colsmart_snc_linea_base.z_b_construccion_casoocho c
	JOIN colsmart_snc_linea_base.z_b_construccion_terreno c0
	  ON c.objectid = c0.objectid
	JOIN colsmart_snc_linea_base.z_u_r_unidad_data u3
	  ON u3.id_caracteristicasunidadconstru = c.id_caracteristicasunidadconstru;
	...
	select caso,count(*)
	from colsmart_snc_linea_base.z_b_construccion_final
	group by caso;
 ...
 */
	...
	from colsmart_snc_linea_base.z_u_r_unidad_data
