 ...
 *
 */
drop table colsmart_prod_insumos.z_f_foliosmatricula_engloble;


create table colsmart_prod_insumos.z_f_foliosmatricula_engloble  as
select *
from colsmart_prod_insumos.z_f_foliosmatricula_3 f
where novedad_numero_predial 
in ('Englobe_Nuevo_FMI');
 ...
 *
 */
drop table colsmart_prod_insumos.z_f_foliosmatricula_engloble_exists;


create table colsmart_prod_insumos.z_f_foliosmatricula_engloble_exists  as
select distinct on (folio_derivados) *
from colsmart_prod_insumos.z_f_foliosmatricula_engloble f
where  exists (
	select 1
...
select count(*)
from  colsmart_prod_insumos.z_f_foliosmatricula_engloble_exists

/****
 * Revisa si los predios no existen
 */
drop table colsmart_prod_insumos.z_f_foliosmatricula_engloble_notexists;


create table colsmart_prod_insumos.z_f_foliosmatricula_engloble_notexists  as
select distinct on (folio_derivados) *
from colsmart_prod_insumos.z_f_foliosmatricula_engloble f
where not exists (
	select 1
...
/****
 * crea la tabla temporal los predios  no existen
 */
	...
	WITH numerados AS (
		...
		FROM colsmart_prod_insumos.z_f_foliosmatricula_engloble_notexists f
		where not exists (
			select 1
		...
		f.npnmatriz npm
		FROM numerados AS n, colsmart_prod_insumos.z_f_foliosmatricula_engloble_notexists f
		WHERE f.id = n.id
	), rep as (
 ...
 */
	...
	WITH src AS (
		select distinct on (pr.matricula_inmobiliaria)
		    ...
		    f.novedad_numero_predial as novedad_numero_tipo_raw,
		    pr.numero_predial_nacional as novedad_numero_predial
		  FROM colsmart_prod_insumos.z_f_foliosmatricula_engloble f
		  JOIN colsmart_preprod_migra.ilc_predio pr
		  ON f.folio_derivados = pr.codigo_orip || '-' || pr.matricula_inmobiliaria
	),
	...
	select observaciones,novedad_numero_tipo,count(*) 
	from  colsmart_preprod_migra.ilc_datosadicionaleslevantamientocatastral
	group by observaciones,novedad_numero_tipo
