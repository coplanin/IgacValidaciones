...
/****
 *  PILAS
 * 	CAMBIAR EL GURPO DE LOTE EN LA CONSYLA
 * 
 */

/****
 * calidad de folios matriz vacios o sin novidedades 
 */

drop table colsmart_prod_insumos.z_f_foliosmatricula_calidad;


create table colsmart_prod_insumos.z_f_foliosmatricula_calidad  as
select *
from colsmart_prod_insumos.z_f_foliosmatricula_3 f
where folio_matriz in ('0','-')  or
COALESCE(novedadfmi_tipo,'')||COALESCE(novedad_numero_predial,'')='';


select *
from colsmart_prod_insumos.z_f_foliosmatricula_calidad;
 ...
 *
 */
drop table colsmart_prod_insumos.z_f_foliosmatricula_cancelacion;


create table colsmart_prod_insumos.z_f_foliosmatricula_cancelacion  as
select *
from colsmart_prod_insumos.z_f_foliosmatricula_3 f
where novedad_numero_predial 
in ('Cancelacion_por_Englobe','Cancelacion_por_Desenglobe','Cancelacion',
'Englobe_Nuevo_FMI');


select *
from colsmart_prod_insumos.z_f_foliosmatricula_cancelacion;

/****
 * Revisa si los predios existen ya creados
 *
 */
drop table colsmart_prod_insumos.z_f_foliosmatricula_cancelacion_notexists;


create table colsmart_prod_insumos.z_f_foliosmatricula_cancelacion_notexists  as
select *
from colsmart_prod_insumos.z_f_foliosmatricula_3 f
where novedad_numero_predial 
in ('Cancelacion_por_Englobe','Cancelacion_por_Desenglobe','Cancelacion',
...
and not  exists (
	...
	where  f.folio_matriz=pr.codigo_orip||'-'||pr.matricula_inmobiliaria
);

select *
from colsmart_prod_insumos.z_f_foliosmatricula_cancelacion_notexists

/****
 * Revisa si los predios  existen
 */
drop table colsmart_prod_insumos.z_f_foliosmatricula_cancelacion_exists;


create table colsmart_prod_insumos.z_f_foliosmatricula_cancelacion_exists  as
select *
from colsmart_prod_insumos.z_f_foliosmatricula_3 f
where novedad_numero_predial 
in ('Cancelacion_por_Englobe','Cancelacion_por_Desenglobe','Cancelacion',
...
select * --count(*)
from  colsmart_prod_insumos.z_f_foliosmatricula_cancelacion_exists 

/****
 * Crea lo predios que no existan para poder cancelarlos desde predio matriz
 */
	...
	WITH numerados AS (
		SELECT
		    ...
		    ) AS rn, "condicion prop derivado"::text condicion_predio                                            -- 1,2,3…
		FROM colsmart_prod_insumos.z_f_foliosmatricula_cancelacion_notexists f
		where not exists (
			select 1
			from colsmart_preprod_migra.ilc_predio p
			where f.folio_matriz=p.codigo_orip||'-'||p.matricula_inmobiliaria
		)--2942 ya creados
		...
		f."condicion prop derivado" condicion,		
		f.coeficiente,
		f.novedadfmi_tipo,
		...
		f.npnmatriz npm
		FROM numerados AS n, colsmart_prod_insumos.z_f_foliosmatricula_cancelacion_notexists f
		WHERE f.id = n.id
	), rep as (
	...
	f.novedadfmi_tipo,
	f.area_registral,
	p.tipo_referencia_fmi_antiguo,
	f.coeficiente,
	f.area_coeficiente,
	sde.next_globalid() globalid,
	CASE
     when left(p.numero_predial_nacional,5)='SIN N'  THEN r.report_muni
     when p.numero_predial_nacional is null  and r.report_muni is not null THEN r.report_muni
     when r.report_muni is null  and left(f.npm,5)!='SIN N' THEN left(f.npm,5)    
	...
	SELECT
	  ...
	  id_operacion,
	  codigo_orip,
	  matricula_inmobiliaria_antigua,
...
/********************************************
 * se llena ILC_FUENTE_ADMINISTRATIVA
 ***************************************/
 ...
 */
	...
	WITH src AS (
	  SELECT
		...
		'L7_'||f.id::text as observaciones,
		...
		f.novedadfmi_tipo as novedadfmi_tipo_raw,
		CASE
		  WHEN f.novedad_numero_predial = 'Englobe_Nuevo_FMI' THEN 'Cancelacion_por_Englobe'
		  ELSE f.novedad_numero_predial
		END AS novedad_numero_tipo_raw,
	    pr.numero_predial_nacional as novedad_numero_predial
	  FROM colsmart_prod_insumos.z_f_foliosmatricula_cancelacion f
	  JOIN colsmart_preprod_migra.ilc_predio pr
	  ON f.folio_matriz = pr.codigo_orip || '-' || pr.matricula_inmobiliaria	  
	),
	...
	norm AS (
	  SELECT
	    ...
	    ''::text                  AS domicilio_notificaciones, -- 🔁 usa j.domicilio_notif si existe; si no, la dirección
	    -- Fecha de visita
	    ...
	    COALESCE(NULLIF(btrim(src.novedad_numero_predial::text),''), 'NO_APLICA') AS novedad_numero_predial, -- 🔁
	...
	select *
	from colsmart_preprod_migra.ilc_datosadicionaleslevantamientocatastral pr
	where exists (	select 1 
			from colsmart_prod_insumos.z_f_predionuevo n	
			where pr.id_operacion_predio=n.id_operacion);
