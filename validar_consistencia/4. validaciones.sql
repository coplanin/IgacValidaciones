...
DROP TABLE IF EXISTS reglas_prod.regla_672;

CREATE TABLE reglas_prod.regla_672 AS
WITH base AS (
  SELECT
    p.objectid,
    p.globalid,
...
-- 691
DROP TABLE IF EXISTS reglas_prod.regla_691;

CREATE TABLE reglas_prod.regla_691 AS
WITH req AS (
  SELECT
    p.objectid,
    p.id_operacion,
...
uc AS (
  ...
  FROM prod.t_cr_unidadconstruccion c
  GROUP BY 1
)
SELECT
  '691'::text                        AS regla,
...
--regla 681
drop table if exists reglas_prod.regla_681;
create table reglas_prod.regla_681 as
WITH p AS (
  SELECT
    objectid,
    id_operacion,
...
DROP TABLE IF EXISTS reglas_prod.regla_685;
CREATE TABLE reglas_prod.regla_685 AS
WITH agg AS (
  SELECT
...
--regla 686
drop table if exists reglas_prod.regla_686;
create table reglas_prod.regla_686 as
WITH t AS (
  SELECT
    p.objectid,
    p.globalid,
...
--regla 688
DROP TABLE IF EXISTS reglas_prod.regla_688;
CREATE TABLE reglas_prod.regla_688 AS
WITH t AS (
  SELECT
    p.objectid,
    p.globalid,
...
--regla 689
DROP TABLE IF EXISTS reglas_prod.regla_689;
create table reglas_prod.regla_689 as
...
--regla 690
DROP TABLE IF EXISTS reglas_prod.regla_690;
CREATE TABLE reglas_prod.regla_690 AS
...
DROP TABLE IF EXISTS reglas_prod.regla_673;

CREATE TABLE reglas_prod.regla_673 AS
WITH union_src AS (
  -- Fuente 1: ILC_Predio
  SELECT 
    'ILC_Predio'::text AS objeto,
    'prod.t_ilc_predio'::text AS tabla,
    p.objectid,
    p.globalid,
    btrim(p.id_operacion) AS id_operacion,
  ...
  SELECT 
    ...
    'prod.t_ilc_datosadicionaleslevantamientocatastral'::text AS tabla,
    d.objectid,
    d.globalid,
    btrim(d.id_operacion_predio) AS id_operacion,
    d.novedad_numero_predial::text AS numero_predial
  FROM prod.t_ilc_datosadicionaleslevantamientocatastral d
),
...
--692
DROP TABLE IF EXISTS reglas_prod.regla_692;

CREATE TABLE reglas_prod.regla_692 AS
WITH base AS (
  SELECT
    p.objectid,
    p.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_693;

CREATE TABLE reglas_prod.regla_693 AS
...
--694
DROP TABLE IF EXISTS prod.t_cr_predio_copropiedad;

CREATE TABLE prod.t_cr_predio_copropiedad (
...
-- Regla 694
DROP TABLE IF EXISTS reglas_prod.regla_694;

CREATE TABLE reglas_prod.regla_694 AS
WITH base AS (
  SELECT
    p.objectid,
    p.globalid,
...
copro AS (
  ...
  FROM prod.t_cr_predio_copropiedad c
...
DROP TABLE IF EXISTS reglas_prod.regla_698;

CREATE TABLE reglas_prod.regla_698 AS
WITH predio_base AS (
  SELECT
    p.objectid,
    p.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_699;

CREATE TABLE reglas_prod.regla_699 AS
...
DROP TABLE IF EXISTS reglas_prod.regla_700;

CREATE TABLE reglas_prod.regla_700 AS
WITH predio_base AS (
  SELECT
    p.objectid,
    p.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_675;

CREATE TABLE reglas_prod.regla_675 AS
WITH base AS (
  SELECT
    p.objectid,
    p.globalid,
...
--674
DROP TABLE IF EXISTS reglas_prod.regla_674;

CREATE TABLE reglas_prod.regla_674 AS
WITH base AS (
  SELECT
    p.objectid,
    p.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_701;

CREATE TABLE reglas_prod.regla_701 AS
WITH params AS (
...
areas_uc AS (   -- Suma áreas privadas UC (CR_UnidadConstruccion)
  ...
  FROM unidades un
  JOIN prod.t_cr_unidadconstruccion uc
...
DROP TABLE IF EXISTS reglas_prod.regla_702;

CREATE TABLE reglas_prod.regla_702 AS
WITH params AS (
  SELECT 0.01::numeric AS tol
),
matrices AS (  -- Predios PH.Matriz (dos condiciones)
...
DROP TABLE IF EXISTS reglas_prod.regla_703;

CREATE TABLE reglas_prod.regla_703 AS
...
--704
DROP TABLE IF EXISTS reglas_prod.regla_704;

CREATE TABLE reglas_prod.regla_704 AS
...
--706
DROP TABLE IF EXISTS reglas_prod.regla_706;

CREATE TABLE reglas_prod.regla_706 AS
WITH params AS (
  SELECT 0.01::numeric AS tol, 9377::int AS srid_m2
),
...
DROP TABLE IF EXISTS reglas_prod.regla_707;

CREATE TABLE reglas_prod.regla_707 AS
WITH params AS (
  SELECT 0.01::numeric AS tol, 9377::int AS srid_m2
),
matrices AS (  -- Predios Condominio_Matriz (dos condiciones)
...
DROP TABLE IF EXISTS reglas_prod.regla_708;

CREATE TABLE reglas_prod.regla_708 AS
WITH params AS (
  SELECT 0.01::numeric AS tol
),
matrices AS (  -- Predios Condominio_Matriz (dos condiciones)
...
DROP TABLE IF EXISTS reglas_prod.regla_709;

CREATE TABLE reglas_prod.regla_709 AS
WITH params AS (
  SELECT 0.01::numeric AS tol
),
matrices AS (  -- Predios Condominio_Matriz (dos condiciones)
...
DROP TABLE IF EXISTS reglas_prod.regla_710;

CREATE TABLE reglas_prod.regla_710 AS
WITH params AS (
  SELECT 0.01::numeric AS tol
),
matrices AS (  -- Predios Condominio_Matriz (dos condiciones)
...
datos_ph AS (  -- Área total construida COMÚN reportada
  SELECT
    ...
    SUM(d.area_total_construida_comun)::numeric AS area_comun_reportada
  FROM prod.t_cr_datosphcondominio d
  JOIN prod.t_ilc_predio p  -- Relacionando con ILC_Predio
...
DROP TABLE IF EXISTS reglas_prod.regla_711;

CREATE TABLE reglas_prod.regla_711 AS
...
DROP TABLE IF EXISTS reglas_prod.regla_712;

CREATE TABLE reglas_prod.regla_712 AS
...
DROP TABLE IF EXISTS reglas_prod.regla_713;

CREATE TABLE reglas_prod.regla_713 AS
...
SELECT
  '713'::text                   AS regla,
  't_ilc_Predio'::text            AS objeto,
  'prod.t_extdireccion'::text  AS tabla,
  pr.objectid,
  pr.globalid,
  d.id_operacion,
  pr.npn,
...
DROP TABLE IF EXISTS reglas_prod.regla_714;

CREATE TABLE reglas_prod.regla_714 AS
WITH base AS (
  SELECT
    d.objectid,
    d.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_715;

CREATE TABLE reglas_prod.regla_715 AS
WITH base AS (
  SELECT
    d.objectid,
    d.globalid,
...
--718
DROP TABLE IF EXISTS reglas_prod.regla_718;

CREATE TABLE reglas_prod.regla_718 AS
WITH base AS (
  SELECT
    d.objectid,
    d.globalid,
    p.id_operacion AS id_operacion,  -- Usamos id_operacion de ILC_Predio asociado a globalid
  ...
  FROM prod.t_extdireccion d
  JOIN prod.t_ilc_predio p ON p.globalid = d.predio_guid  -- Enlace con ILC_Predio por globalid
  WHERE TRIM(d.tipo_direccion::text) = 'Estructurada'
),
flags AS (
  SELECT
...
SELECT
  ...
  f.objectid,
  f.globalid,
  f.id_operacion,  -- Aquí usamos id_operacion obtenido del JOIN con ILC_Predio
  NULL::text AS npn,
  CONCAT_WS('; ',
...
--719
DROP TABLE IF EXISTS reglas_prod.regla_719;

CREATE TABLE reglas_prod.regla_719 AS
WITH branch_info AS (
  SELECT
    p.globalid,
    p.id_operacion,  -- No necesitamos btrim() para id_operacion
    p.gdb_branch_id  -- Usamos gdb_branch_id directamente
  FROM prod.t_ilc_predio p
)
SELECT
  '719'::text                                              AS regla,
  ...
  d.objectid,
  d.globalid,
  bi.id_operacion                                          AS id_operacion,  -- Usamos predio_guid para enlazar id_operacion
  NULL::text                                               AS npn,
  char_length(d.observaciones)                             AS valor,
  'INCUMPLE: Observaciones supera los 254 caracteres ('|| char_length(d.observaciones) ||')' AS descripcion,
  FALSE                                                    AS cumple,
  NOW() AS created_at,
  NOW() AS updated_at
FROM prod.t_ilc_datosadicionaleslevantamientocatastral d
JOIN prod.t_ilc_predio p ON p.globalid = d.predio_guid  -- Haciendo el join con ILC_Predio usando predio_guid
JOIN branch_info bi ON bi.globalid = p.globalid            -- Enlace con gdb_branch_id
WHERE d.observaciones IS NOT NULL
  AND char_length(d.observaciones) > 254
ORDER BY bi.id_operacion, d.objectid;

--720
DROP TABLE IF EXISTS reglas_prod.regla_720;

CREATE TABLE reglas_prod.regla_720 AS
WITH base AS (
  SELECT
    d.objectid,
    d.globalid,
    p.id_operacion                                           AS id_operacion,  -- Usamos id_operacion de la tabla ILC_Predio
  ...
  FROM prod.t_ilc_datosadicionaleslevantamientocatastral d
  JOIN prod.t_ilc_predio p ON p.globalid = d.predio_guid  -- Join con predio_guid para obtener id_operacion
),
...
--722
DROP TABLE IF EXISTS reglas_prod.regla_722;

CREATE TABLE reglas_prod.regla_722 AS
WITH base AS (
  SELECT
    d.objectid,
    d.globalid,
    d.id_operacion_predio AS id_operacion,
    d.predio_guid,  -- Aquí agregamos predio_guid
    NULLIF(TRIM(d.nombres_apellidos_quien_atendio::text), '') AS nombre_atendio,
    NULLIF(TRIM(d.tipo_doc_quien_atendio::text), '')          AS tipo_doc_atendio,
    NULLIF(TRIM(d.num_doc_quien_atendio::text), '')           AS num_doc_atendio,
...
con_info AS (  -- Filtramos por información válida de la persona y correo
  SELECT *
  FROM base
...
SELECT
  ...
  c.globalid,
  c.id_operacion,
  c.predio_guid,  -- Incluir predio_guid en el SELECT
  NULL::text AS npn,
  c.correo AS valor,
  ...
  NOW() AS updated_at
FROM con_info c
-- Filtramos por correo que no cumple con el patrón
...
-- 723
-- Regla de validación
DROP TABLE IF EXISTS reglas_prod.regla_723;

CREATE TABLE reglas_prod.regla_723 AS
WITH base AS (
  SELECT
    d.objectid,
    d.globalid,
    d.id_operacion_predio AS id_operacion,
    d.predio_guid,   -- Incluimos predio_guid
    NULLIF(TRIM(d.nombres_apellidos_quien_atendio::text), '') AS nombre_atendio,
...
con_nombre_y_autoriza AS (
  SELECT *
  FROM base
  WHERE nombre_atendio IS NOT NULL  -- Solo registros con nombre diligenciado
    AND autoriza = TRUE             -- Y donde autoriza notificaciones es verdadero
)
SELECT
  '723'::text AS regla,
  'ILC_DatosAdicionalesLevantamientoCatastral' AS tabla,
  'Celular_Correo' AS objeto,
  c.objectid,
  c.globalid,
  c.id_operacion,
  c.predio_guid,  -- Incluimos predio_guid
  NULL::text AS npn,
  'celular=' || COALESCE(c.celular, '<NULL>') || ', correo=' || COALESCE(c.correo, '<NULL>') AS valor,
  'INCUMPLE: Si existe información de quien atendió la visita y autoriza notificaciones, entonces celular y/o correo deben estar diligenciados.' AS descripcion,
  FALSE AS cumple,
  NOW() AS created_at,
  NOW() AS updated_at
FROM con_nombre_y_autoriza c
WHERE c.celular IS NULL AND c.correo IS NULL  -- Verificamos que ambos campos estén vacíos
ORDER BY c.id_operacion, c.objectid;



--724
DROP TABLE IF EXISTS reglas_prod.regla_724;

CREATE TABLE reglas_prod.regla_724 AS
WITH base AS (
  SELECT
    d.objectid,
    d.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_716;

CREATE TABLE reglas_prod.regla_716 AS
WITH base AS (
  SELECT
    e.objectid,
    e.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_717;

CREATE TABLE reglas_prod.regla_717 AS
WITH predio AS (
  SELECT
    p.objectid,
    p.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_676;

CREATE TABLE reglas_prod.regla_676 AS
WITH base AS (
  SELECT
    p.objectid,
    p.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_721;

CREATE TABLE reglas_prod.regla_721 AS
WITH base AS (
  SELECT
    d.objectid,
    d.globalid,
...
-- Regla de validación con filtro de gdb_branch_id
DROP TABLE IF EXISTS reglas_prod.regla_725;

CREATE TABLE reglas_prod.regla_725 AS
WITH base AS (
  SELECT
    p.objectid,
    p.globalid,
    p.condicion_predio,  -- Condición del predio
    e.complemento,       -- Complemento de la dirección
    p.id_operacion,
    e.gdb_branch_id      -- Incluimos gdb_branch_id en la selección
  FROM
    prod.t_ilc_predio p
  JOIN prod.t_extdireccion e ON p.globalid = e.predio_guid  -- Relacionamos con t_extdireccion usando globalid
)
SELECT
  '725'::text AS regla,
  'ILC_Predio' AS tabla,
  'ExtDireccion_Complemento' AS objeto,
  b.objectid,
  b.globalid,
  b.id_operacion,
  NULL::text AS npn,
  b.complemento AS valor,
  'INCUMPLE: Si Condicion_Predio es PH.Unidad_Predial o Condominio.Unidad_Predial, Complemento debe contener valores válidos.' AS descripcion,
  FALSE AS cumple,
  NOW() AS created_at,
  NOW() AS updated_at
FROM
  base b
WHERE
  (b.condicion_predio IN ('PH.Unidad_Predial', 'Condominio.Unidad_Predial')  -- Verificamos la condición del predio
   AND b.complemento NOT IN ('AP', 'BQ', 'BD', 'CS', 'ED', 'ET', 'GA', 'IN', 'LO', 'MZ', 'OF', 'PQ', 'PN', 'TO', 'UN', 'UR', 'TO', 'AP'))  -- Verificamos si el complemento tiene los valores válidos
  AND b.gdb_branch_id = 0  -- Filtro por gdb_branch_id igual a 0
ORDER BY b.id_operacion, b.objectid;


--726
DROP TABLE IF EXISTS reglas_prod.regla_726;

CREATE TABLE reglas_prod.regla_726 AS
WITH informales AS (  -- predios informales
  ...
  WHERE p.numero_predial_nacional IS NOT NULL
    ...
    AND substring(p.numero_predial_nacional FROM 22 FOR 1) = '2'  -- Verificamos que el dígito 22 sea '2'
),
links AS (  -- vínculos informal-formal
  ...
  FROM prod.t_ilc_predio_informalidad pi
  JOIN prod.t_ilc_predio pf
    ON pi.predio_formal_guid = pf.globalid  -- Relacionamos por predio_formal_guid y globalid
...
---727
DROP TABLE IF EXISTS reglas_prod.regla_727;

CREATE TABLE reglas_prod.regla_727 AS
WITH informales AS (  -- predios informales
  SELECT
    p.objectid,  -- Usamos objectid para el predio informal
    p.globalid,
    btrim(p.id_operacion) AS id_operacion,
    p.numero_predial_nacional AS npn_inf
  FROM prod.t_ilc_predio p
  WHERE p.numero_predial_nacional IS NOT NULL
    AND char_length(p.numero_predial_nacional) >= 22
    AND substring(p.numero_predial_nacional FROM 22 FOR 1) = '2'  -- Verificamos que el dígito 22 sea '2' (informal)
),
links AS (  -- vínculos informal-formal
  SELECT
    pi.objectid AS predio_informal_guid,  -- Usamos objectid para el predio informal
    array_agg(DISTINCT pf.numero_predial_nacional) AS npn_formales,
    bool_or(substring(pf.numero_predial_nacional FROM 22 FOR 1) <> '2') AS tiene_formal_valido  -- Verificamos que el dígito 22 no sea '2' (formal)
  FROM prod.t_ilc_predio_informalidad pi
  JOIN prod.t_ilc_predio pf
    ON pi.predio_formal_guid = pf.globalid  -- Relacionamos por predio_formal_guid y globalid
  WHERE substring(pf.numero_predial_nacional FROM 22 FOR 1) <> '2'  -- Solo predios formales (NPN[22] debe ser diferente de '2')
  GROUP BY pi.objectid  -- Relacionamos por objectid de predio informal
)
SELECT
  '727'::text AS regla,
  'ILC_Predio_Informalidad'::text AS tabla,
  ...
  NOW() AS updated_at
FROM informales inf
LEFT JOIN links l ON l.predio_informal_guid = inf.objectid  -- Relacionamos por objectid del predio informal
WHERE l.predio_informal_guid IS NULL          -- No tiene relación
   OR l.tiene_formal_valido IS NOT TRUE -- Tiene relaciones pero ninguna a predio formal válido
ORDER BY inf.id_operacion, inf.npn_inf;
...
-- Regla 677
DROP TABLE IF EXISTS reglas_prod.regla_677;

CREATE TABLE reglas_prod.regla_677 AS
WITH base AS (
  SELECT
    p.objectid,
    p.globalid,
...
--678
-- Regla 678 con branch (join correcto usando predio_guid para otras tablas)
DROP TABLE IF EXISTS reglas_prod.regla_678;

CREATE TABLE reglas_prod.regla_678 AS
WITH base AS (
  SELECT
    p.objectid,
    p.globalid,
...
branch AS (
  -- Join con tabla relacionada usando predio_guid
  SELECT
    b.*,
    d.nombres_apellidos_quien_atendio AS nombre_atendio,
    d.tipo_doc_quien_atendio AS tipo_doc_atendio,
    d.num_doc_quien_atendio AS num_doc_atendio
  FROM base b
  LEFT JOIN prod.t_ilc_datosadicionaleslevantamientocatastral d
    ON d.predio_guid = b.globalid  -- join correcto usando predio_guid
),
viol AS (
  SELECT
    br.*,
    (br.ok_formato AND br.s22 = '7') AS ok_22,
    (br.ok_formato AND br.s23_24 <> '00') AS ok_23_24,
    (br.ok_formato AND br.s25_26 <> '00') AS ok_25_26,
    (br.ok_formato AND br.s27_30 <> '0000') AS ok_27_30,
    trim(both ', ' FROM concat_ws(', ',
      CASE WHEN NOT br.ok_formato THEN 'formato inválido: NPN debe tener 30 dígitos' END,
      CASE WHEN br.ok_formato AND NOT (br.s22 = '7') THEN 'pos22≠7' END,
      CASE WHEN br.ok_formato AND NOT (br.s23_24 <> '00') THEN 'pos23-24="00"' END,
      CASE WHEN br.ok_formato AND NOT (br.s25_26 <> '00') THEN 'pos25-26="00"' END,
      CASE WHEN br.ok_formato AND NOT (br.s27_30 <> '0000') THEN 'pos27-30="0000"' END
    )) AS motivo
  FROM branch br
)
SELECT
...
WHERE
  ...
  OR NOT v.ok_27_30
ORDER BY v.id_operacion, v.npn;
-- Regla 679 con branch (join seguro usando predio_guid)
DROP TABLE IF EXISTS reglas_prod.regla_679;

CREATE TABLE reglas_prod.regla_679 AS
WITH base AS (
  SELECT
    p.objectid,
    p.globalid,
...
branch AS (
  -- branch listo para joins futuros usando predio_guid
  SELECT
    b.*,
    d.nombres_apellidos_quien_atendio AS nombre_atendio,
    d.tipo_doc_quien_atendio AS tipo_doc_atendio,
  ...
  FROM base b
  LEFT JOIN prod.t_ilc_datosadicionaleslevantamientocatastral d
    ON d.predio_guid = b.globalid
),
viol AS (
  SELECT
    br.*,
    (br.ok_formato AND br.s22_30 = '800000000') AS ok_bloque,
    trim(both ', ' FROM concat_ws(', ',
      CASE WHEN NOT br.ok_formato THEN 'formato inválido: NPN debe tener 30 dígitos' END,
      CASE WHEN br.ok_formato AND br.s22_30 <> '800000000' THEN 'pos22-30≠800000000' END
    )) AS motivo
  FROM branch br
)
SELECT
  ...
  v.objectid,
  v.globalid,
  v.id_operacion,  -- <-- mantener siempre de la tabla base
  v.npn,
...
-- Regla 680
-- Regla 680 con branch filtrando solo branch = 0
DROP TABLE IF EXISTS reglas_prod.regla_680;

CREATE TABLE reglas_prod.regla_680 AS
WITH base AS (
  SELECT
    p.objectid,
    p.globalid,
...
viol AS (
  SELECT
    br.*,
    (br.ok_formato AND br.s22_26 = '80000' AND br.s27_30 <> '0000') AS ok_bloque,
    trim(both ', ' FROM concat_ws(', ',
      CASE WHEN NOT br.ok_formato THEN 'formato inválido: NPN debe tener 30 dígitos' END,
      CASE WHEN br.ok_formato AND br.s22_26 <> '80000' THEN 'pos22-26≠80000' END,
      CASE WHEN br.ok_formato AND br.s27_30 = '0000' THEN 'pos27-30=0000' END
    )) AS motivo
  FROM branch br
)
SELECT
...
DROP TABLE IF EXISTS reglas_prod.regla_728;

CREATE TABLE reglas_prod.regla_728 AS
WITH base AS (
  SELECT
    t.objectid,
    t.globalid,
    ...
    t.id_operacion_predio  -- Usamos id_operacion_predio para relacionar con los predios
  FROM prod.t_ilc_tramitesderechosterritoriales t
...
SELECT
  ...
  i.motivo AS descripcion,  -- Ahora 'motivo' está calculado en la subconsulta 'inc'
  FALSE AS cumple,
  NOW() AS created_at,
  NOW() AS updated_at
FROM inc i
WHERE i.motivo IS NOT NULL  -- Filtramos solo aquellos registros con 'motivo'
ORDER BY i.id_operacion, i.objectid;

-- Regla 729 con predio_guid y branch-ready
DROP TABLE IF EXISTS reglas_prod.regla_729;

CREATE TABLE reglas_prod.regla_729 AS
WITH predio AS (
  SELECT
    p.objectid                          AS predio_oid,
    p.globalid                          AS predio_guid,  -- <-- para joins futuros
...
derecho AS (
  ...
  FROM prod.t_ilc_derecho d
),
base AS (
  SELECT
    pr.predio_oid,
    pr.predio_guid,                       -- <-- agregamos predio_guid aquí
  ...
  SELECT
    ...
    CASE
      ...
      WHEN f.fecha_inicio_tenencia <> f.fecha_esperada THEN
        'INCUMPLE: Fecha_Inicio_Tenencia='||to_char(f.fecha_inicio_tenencia,'YYYY-MM-DD')||'; se esperaba '||to_char(f.fecha_esperada,'YYYY-MM-DD')
    END AS motivo
  FROM filtro f
)
SELECT
  ...
  'prod.t_ilc_derecho'::text         AS tabla,
  v.predio_oid                          AS objectid,            -- <-- objectid de la tabla predio
  v.predio_guid                         AS globalid,            -- <-- guid del predio para joins
  v.id_operacion_predio                 AS id_operacion,        -- <-- id_operacion de la tabla predio
  v.npn,
...
DROP TABLE IF EXISTS reglas_prod.regla_738;

CREATE TABLE reglas_prod.regla_738 AS
WITH predio AS (
  SELECT
    p.objectid                AS predio_oid,
...
-- Regla 739 con predio_guid (sin filtro branch)

DROP TABLE IF EXISTS reglas_prod.regla_739;

CREATE TABLE reglas_prod.regla_739 AS
WITH pr AS (
  SELECT
    p.objectid,
    p.globalid                          AS predio_guid,       -- <-- agregamos predio_guid
    btrim(p.id_operacion)               AS id_operacion,
    p.numero_predial_nacional           AS npn,
    lower(btrim(p.condicion_predio))    AS cp,
...
eval AS (
  SELECT
    pr.objectid,
    pr.predio_guid,                     -- <-- usamos predio_guid
    pr.id_operacion,
    pr.npn,
    pr.cp,
    pr.tipo_predio,
...
SELECT
  ...
  'prod.t_ilc_predio'::text         AS tabla,
  v.objectid,
  v.predio_guid                         AS globalid,        -- <-- regresamos predio_guid
  v.id_operacion,
  v.npn,
...
DROP TABLE IF EXISTS reglas_prod.regla_740;

CREATE TABLE reglas_prod.regla_740 AS
WITH base AS (
  SELECT 
  ...
  LEFT JOIN prod.t_ilc_fuenteadministrativa f 
...
--741
-- Regla 741 con id_operacion_predio
DROP TABLE IF EXISTS reglas_prod.regla_741;

CREATE TABLE reglas_prod.regla_741 AS
WITH predios AS (
  SELECT
    p.objectid,
    p.globalid,
...
fuentes AS (
  ...
  FROM prod.t_ilc_fuenteadministrativa f
),
visita AS (
  SELECT
...
SELECT
  ...
  a.objectid,
  a.globalid,
  a.id_operacion,  -- <-- id_operacion del predio
...
--742
--742 con predio_guid
DROP TABLE IF EXISTS reglas_prod.regla_742;

CREATE TABLE reglas_prod.regla_742 AS
WITH base AS (
  SELECT
    i.objectid,
    i.globalid,
...
viol AS (
  SELECT
    ...
    pg.predio_guid
  FROM base b
  LEFT JOIN predio_guid pg
    ON pg.id_operacion_predio = b.id_operacion_predio
...
SELECT
  ...
  'prod.t_ilc_interesado'::text       AS tabla,
  v.objectid,
  v.globalid,
  v.predio_guid                          AS globalid_predio,  -- <-- ahora usamos predio_guid
  ...
  NOW()                                   AS updated_at
FROM viol v
ORDER BY v.predio_guid, v.objectid;

--743
--743 con predio_guid
DROP TABLE IF EXISTS reglas_prod.regla_743;

CREATE TABLE reglas_prod.regla_743 AS
WITH base AS (
  SELECT
    i.objectid,
    i.globalid,
...
SELECT
  ...
  COALESCE(v.tipo_documento,'(null)')::text AS valor,   -- el valor malo (NIT)
  FALSE                           AS cumple,
  NOW()                           AS created_at,
  NOW()                           AS updated_at
FROM viol v
...
--744
--744 con predio_guid
DROP TABLE IF EXISTS reglas_prod.regla_744;

CREATE TABLE reglas_prod.regla_744 AS
WITH base AS (
  SELECT
    i.objectid,
    i.globalid,
...
SELECT
  ...
  v.objectid,
  v.globalid,
  v.predio_guid                   AS globalid_predio, -- usamos predio_guid
  ...
  NOW()                           AS updated_at
FROM viol v
WHERE v.predio_guid IS NOT NULL
ORDER BY v.predio_guid, v.objectid;
--745 con predio_guid
DROP TABLE IF EXISTS reglas_prod.regla_745;

CREATE TABLE reglas_prod.regla_745 AS
WITH base AS (
  SELECT
    i.objectid,
    i.globalid,
...
ORDER BY v.predio_guid, v.objectid;
--746 con predio_guid
DROP TABLE IF EXISTS reglas_prod.regla_746;

CREATE TABLE reglas_prod.regla_746 AS
WITH base AS (
  SELECT
    i.objectid,
    i.globalid,
...
SELECT
  ...
  v.objectid,
  v.globalid,
  v.predio_guid                   AS globalid_predio,  -- usando predio_guid
  NULL::varchar(30)               AS npn, -- no aplica
...
--747 con predio_guid
DROP TABLE IF EXISTS reglas_prod.regla_747;

CREATE TABLE reglas_prod.regla_747 AS
WITH base AS (
  SELECT
    i.objectid,
    i.globalid,
...
--- regla 730
--730 con predio_guid
DROP TABLE IF EXISTS reglas_prod.regla_730;

CREATE TABLE reglas_prod.regla_730 AS
WITH base AS (
  SELECT
    d.objectid,
    d.globalid,
...
--748
DROP TABLE IF EXISTS reglas_prod.regla_748;

CREATE TABLE reglas_prod.regla_748 AS
WITH base AS (
  SELECT
    i.objectid,
    i.globalid,
...
ORDER BY v.id_operacion_predio, v.objectid;
--749 con predio_guid
DROP TABLE IF EXISTS reglas_prod.regla_749;

CREATE TABLE reglas_prod.regla_749 AS
WITH base AS (
  SELECT
    i.objectid,
    i.globalid,
...
--750 con predio_guid y ajuste en la relación de id_operacion_predio
DROP TABLE IF EXISTS reglas_prod.regla_750;

CREATE TABLE reglas_prod.regla_750 AS
WITH base AS (
  SELECT
    i.objectid,
    i.globalid,
...
-- Relacionamos con t_ilc_predio para obtener el id_operacion_predio desde el predio_guid
predio_guid AS (
  SELECT 
    p.objectid AS predio_oid,
    p.globalid AS predio_guid, -- Utilizamos globalid de t_ilc_predio
    p.id_operacion AS id_operacion_predio
  FROM prod.t_ilc_predio p
),
viol AS (
  SELECT
    b.*,
    pg.predio_guid,  -- Añadimos predio_guid de la relación
    CASE 
      WHEN b.sexo IS NOT NULL AND b.tipo <> 'PERSONA_NATURAL'
      THEN 'Sexo='||COALESCE(b.sexo,'(null)')||', Tipo='||COALESCE(b.tipo,'(null)')
    END AS motivo
  FROM base b
  LEFT JOIN predio_guid pg ON pg.predio_guid = b.globalid -- Relación entre predio_guid y globalid
  WHERE b.sexo IS NOT NULL
    AND b.tipo <> 'PERSONA_NATURAL'  -- Filtro donde 'sexo' está diligenciado y 'tipo' no es 'PERSONA_NATURAL'
)
SELECT
  '750'::text AS regla,
  't_ilc_Interesado'::text AS objeto,
  ...
  v.objectid,
  v.globalid,
  v.id_operacion_predio,  -- Mostramos id_operacion_predio obtenido desde la relación
  v.predio_guid,           -- Incluimos el predio_guid
  NULL::varchar(30) AS npn,  -- No aplica en este caso
...
-- 751: Regla con id_operacion_predio y branch
DROP TABLE IF EXISTS reglas_prod.regla_751;

CREATE TABLE reglas_prod.regla_751 AS
WITH base AS (
  SELECT
    i.objectid,
    i.globalid,
...
-- Relacionamos con t_ilc_predio para obtener id_operacion_predio desde predio_guid
predio_guid AS (
  SELECT 
    p.objectid AS predio_oid,
    p.globalid AS predio_guid, 
...
flag AS (
  SELECT
    n.*,
    -- Marcas societarias en primer nombre, segundo nombre, primer apellido, segundo apellido
...
SELECT
  ...
  v.objectid,
  v.globalid,
  v.id_operacion_predio           AS id_operacion,  -- Obtención del id_operacion_predio
  NULL::varchar(30)               AS npn,  -- no aplica
  ...
  array_to_string(v.motivos, ', ')::text   AS valor,  -- Lista de "campo contiene TOKEN (valor=...)"
  FALSE                           AS cumple,
  NOW()                           AS created_at,
  NOW()                           AS updated_at,
  -- Branch con lógica condicional
  CASE
    WHEN v.id_operacion_predio IS NULL THEN 'Sin operación asociada'
    ELSE 'Operación asociada'
  END AS branch_result -- Branch en base a si id_operacion_predio es NULL o no
FROM viol v
WHERE array_length(v.motivos, 1) > 0
ORDER BY v.id_operacion_predio, v.objectid;
...
DROP TABLE IF EXISTS reglas_prod.regla_757;

CREATE TABLE reglas_prod.regla_757 AS
WITH base AS (
  SELECT
    f.objectid,
    f.globalid,
...
SELECT
  ...
  'prod.t_ilc_fuenteadministrativa'::text AS tabla,
  v.objectid,
  v.globalid,
  v.id_operacion_predio               AS id_operacion,
...
/*DROP TABLE IF EXISTS reglas_prod.regla_752;

CREATE TABLE reglas_prod.regla_752 AS
WITH base AS (
  SELECT
    a.objectid,
    a.globalid,
    ...
    i.tipo                   AS tipo_interesado
  FROM prod.t_cr_agrupacioninteresados a
  JOIN prod.t_ilc_interesado i
...
DROP TABLE IF EXISTS reglas_prod.regla_753;

CREATE TABLE reglas_prod.regla_753 AS
WITH base AS (
  SELECT
    a.objectid,
    a.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_754;

CREATE TABLE reglas_prod.regla_754 AS
WITH base AS (
  SELECT
    a.objectid,
    a.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_755;

CREATE TABLE reglas_prod.regla_755 AS
WITH base AS (
  SELECT
    m.objectid,
    m.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_756;

CREATE TABLE reglas_prod.regla_756 AS
WITH base AS (
  SELECT
    f.objectid,
    f.globalid,
...
*/
--731
DROP TABLE IF EXISTS reglas_prod.regla_731;

CREATE TABLE reglas_prod.regla_731 AS
WITH base AS (
    SELECT
        d.objectid,
        d.globalid,
    ...
    FROM prod.t_ilc_derecho d
    JOIN prod.t_ilc_predio p
      ON p.id_operacion = d.id_operacion_predio  -- Relacionamos por id_operacion
),
predio_guid AS (
  SELECT 
    p.id_operacion AS id_operacion_predio,
    p.globalid     AS predio_guid  -- Obtenemos el predio_guid
  FROM prod.t_ilc_predio p
),
viol AS (
    SELECT
        b.*,
        pg.predio_guid,  -- Añadimos el predio_guid a la selección
        CASE 
    ...
    FROM base b
    LEFT JOIN predio_guid pg
        ON pg.id_operacion_predio = b.id_operacion_predio -- Relacionamos por id_operacion_predio
)
SELECT
    '731'::text                       AS regla,
    't_ilc_Derecho'::text             AS objeto,
    'prod.t_ilc_derecho'::text     AS tabla,
    MAX(v.objectid) AS objectid,  -- Usamos MAX() para objectid
    MAX(v.globalid) AS globalid,  -- Usamos MAX() para globalid
    v.id_operacion_predio            AS id_operacion,  -- Agrupamos por id_operacion_predio
    v.predio_guid                    AS globalid_predio,  -- Incluimos el predio_guid
    NULL::varchar(30)                 AS npn, -- no aplica aquí
    MAX('INCUMPLE: ' || v.motivo)::text AS descripcion,  -- Usamos MAX() para motivo
    -- Concatenamos los derechos que no cuadran (sin repetir predio_tipo)
    'derechos: ' || string_agg(
        COALESCE(v.derecho_tipo, '(null)'), ', '
    )::text || ', predio_tipo=' || MAX(COALESCE(v.predio_tipo, '(null)')) AS valor,  -- Concatenamos derechos y solo un predio_tipo
    FALSE                             AS cumple,
    NOW()                             AS created_at,
    NOW()                             AS updated_at
FROM viol v
WHERE v.motivo IS NOT NULL
GROUP BY v.id_operacion_predio, v.predio_guid  -- Agrupamos solo por id_operacion_predio y predio_guid
ORDER BY v.id_operacion_predio, objectid;

--758
--758 sin filtro branch
DROP TABLE IF EXISTS reglas_prod.regla_758;

CREATE TABLE reglas_prod.regla_758 AS
WITH base AS (
  SELECT
    p.objectid,
    p.globalid,
...
agg AS (
  ...
  FROM base b
  LEFT JOIN prod.t_ilc_interesado i
    ON b.id_operacion_predio = btrim(i.id_operacion_predio) -- Relacionamos por id_operacion_predio
...
SELECT
  ...
  a.objectid,
  a.globalid,
  a.id_operacion_predio        AS id_operacion,  -- Mostramos id_operacion_predio
  a.npn,
...
--759 con predio_guid para reglas_prod
DROP TABLE IF EXISTS reglas_prod.regla_759;

CREATE TABLE reglas_prod.regla_759 AS
WITH base AS (
    SELECT
        i.objectid AS objectid_interesado,
        i.globalid AS globalid_interesado,
        btrim(i.id_operacion_predio) AS id_operacion_predio,
        ...
        p.numero_predial_nacional AS npn,
        p.id_operacion AS id_operacion_predio_real
    FROM prod.t_ilc_interesado i
    LEFT JOIN prod.t_ilc_predio p
        ON p.id_operacion = i.id_operacion_predio
),
agrupado AS (
    SELECT
        primer_nombre,
        ...
        ARRAY_AGG(DISTINCT id_operacion_predio_real) AS predios,
        ARRAY_AGG(DISTINCT npn) AS npns
    FROM base
...
SELECT
    ...
    array_to_string(a.predios, ',') AS id_operacion,  -- aquí llenamos id_operacion
    array_to_string(a.npns, ',') AS npn,             -- aquí llenamos npn
    'INCUMPLE: Existen ' || a.n_docs || ' documentos diferentes asociados a mismos nombres/razón social' AS descripcion,
    'Nombres=' || COALESCE(NULLIF(a.primer_nombre,''),'(null)') || ' ' ||
    ...
    ', NPN=' || array_to_string(a.npns, ',') AS valor,
    FALSE AS cumple,
    NOW() AS created_at,
    NOW() AS updated_at
FROM agrupado a;
...
--760 con id_operacion_predio obtenido por predio_guid
-- 760: Documento_Identidad duplicado (sin branch)
DROP TABLE IF EXISTS reglas_prod.regla_760;

CREATE TABLE reglas_prod.regla_760 AS
WITH interesado AS (
  SELECT
    i.objectid,
    i.globalid,
...
base AS (
  SELECT
    ...
    it.id_operacion_predio,
    pr.id_operacion,
    pr.npn
  FROM interesado it
  LEFT JOIN predio pr
    ON pr.id_operacion = it.id_operacion_predio
),
duplicados AS (
  SELECT
    b.documento_identidad,
    COUNT(*)                                   AS n_veces,
    ARRAY_AGG(DISTINCT b.objectid)             AS objectids_interesado,
    ARRAY_AGG(DISTINCT b.globalid)             AS globalids_interesado,
    ARRAY_AGG(DISTINCT b.id_operacion_predio)  AS id_ops_interesado,
    ARRAY_AGG(DISTINCT b.id_operacion)         AS id_ops,
    ARRAY_AGG(DISTINCT b.npn)                  AS npns
  FROM base b
  WHERE b.documento_identidad IS NOT NULL
  GROUP BY b.documento_identidad
  HAVING COUNT(*) > 1
)
SELECT
  '760'::text                           AS regla,
  ...
  NULL::bigint                          AS objectid,
  NULL::uuid                            AS globalid,
  array_to_string(d.id_ops, ',')        AS id_operacion,
  array_to_string(d.npns, ',')          AS npn,
  ...
  (
    ...
    ', ObjectIDs_Interesado='||array_to_string(d.objectids_interesado, ',')||
    ', GlobalIDs_Interesado='||array_to_string(d.globalids_interesado, ',')||
    ', Id_Operaciones='||array_to_string(d.id_ops, ',')||
    ', NPN='||array_to_string(d.npns, ',')||
    ', Id_Operaciones_Interesado='||array_to_string(d.id_ops_interesado, ',')
  )::text                             AS valor,
  FALSE                                AS cumple,
  NOW()                                AS created_at,
  NOW()                                AS updated_at
...
--761
DROP TABLE IF EXISTS reglas_prod.regla_761;

CREATE TABLE reglas_prod.regla_761 AS
WITH base AS (
  SELECT
    i.objectid,
    i.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_732;

CREATE TABLE reglas_prod.regla_732 AS
WITH base AS (
  SELECT
    d.objectid,
    d.globalid,
...
--733
DROP TABLE IF EXISTS reglas_prod.regla_733;

CREATE TABLE reglas_prod.regla_733 AS
WITH base AS (
  SELECT
    d.objectid,
    d.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_734;

CREATE TABLE reglas_prod.regla_734 AS
WITH dpr AS (
  SELECT
    d.objectid,
    d.globalid,
...
--735
/*
DROP TABLE IF EXISTS reglas_prod.regla_735;

CREATE TABLE reglas_prod.regla_735 AS
WITH predios AS (
  SELECT
    p.objectid,
    p.globalid,
...
--737
DROP TABLE IF EXISTS reglas_prod.regla_737;

CREATE TABLE reglas_prod.regla_737 AS
WITH dpr AS (
    SELECT
        d.objectid,
        d.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_762_incumple;

CREATE TABLE reglas_prod.regla_762_incumple AS
WITH predios AS (
  SELECT
    p.objectid,
    p.globalid,
...
cuc AS (  -- características (uso)
  SELECT
    ...
    c.uso
  FROM prod.t_ilc_caracteristicasunidadconstruccion c
),
...
--774
DROP TABLE IF EXISTS reglas_prod.regla_774;

CREATE TABLE reglas_prod.regla_774 AS
WITH uc AS (
  SELECT
    u.objectid,
    u.globalid,
...
SELECT
  ...
  'prod.t_cr_unidadconstruccion'::text AS tabla,
  uc.objectid,
  uc.globalid,
  uc.id_operacion,
...
--771
DROP TABLE IF EXISTS reglas_prod.regla_771;

CREATE TABLE reglas_prod.regla_771 AS
WITH cuc AS (
  SELECT
    c.objectid,
    c.globalid,
...
SELECT
  ...
  'prod.t_ilc_caracteristicasunidadconstruccion'::text    AS tabla,
  cuc_uc.objectid,
  cuc_uc.globalid,
  cuc_uc.id_operacion,
  NULL::varchar(30)                                        AS npn,
...
--772
DROP TABLE IF EXISTS reglas_prod.regla_772;

CREATE TABLE reglas_prod.regla_772 AS
WITH cuc AS (
  SELECT
    c.objectid,
    c.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_773;

CREATE TABLE reglas_prod.regla_773 AS
WITH cuc AS (
  SELECT
    c.objectid,
    c.globalid,
...
--775
DROP TABLE IF EXISTS reglas_prod.regla_775_incumple;

CREATE TABLE reglas_prod.regla_775_incumple AS
WITH predio AS (
  SELECT
    p.objectid,
    p.globalid,
...
--776
DROP TABLE IF EXISTS reglas_prod.regla_776_incumple;

CREATE TABLE reglas_prod.regla_776_incumple AS
WITH predio AS (
  SELECT
    p.objectid,
    p.globalid,
...
--777
DROP TABLE IF EXISTS reglas_prod.regla_777;

CREATE TABLE reglas_prod.regla_777 AS
WITH predio AS (
  SELECT
    p.objectid,
    p.globalid,
...
--778
DROP TABLE IF EXISTS reglas_prod.regla_778_incumple;

CREATE TABLE reglas_prod.regla_778_incumple AS
WITH predio AS (
  SELECT
    p.objectid,
    p.globalid,
...
DROP TABLE IF EXISTS reglas_prod.779;

CREATE TABLE reglas_prod.779 AS
WITH cuc AS (
  SELECT
    c.objectid,
    c.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_780;

CREATE TABLE reglas_prod.regla_780 AS
WITH predio AS (
  SELECT
    p.objectid,
    p.globalid,
...
DROP TABLE IF EXISTS reglas_prod.763;

CREATE TABLE reglas_prod.763 AS
WITH predio AS (
  SELECT
    p.objectid,
    p.globalid,
...
DROP TABLE IF EXISTS reglas_prod.781;

CREATE TABLE reglas_prod.781 AS
WITH predio AS (
  SELECT
    p.objectid,
    p.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_782;

CREATE TABLE reglas_prod.regla_782 AS
WITH predio AS (
  SELECT
    p.objectid,
    p.globalid,
...
--783
ROLLBACK;
DROP TABLE IF exists reglas_prod.regla_783;
CREATE TABLE reglas_prod.regla_783 AS
WITH params AS (
...
ROLLBACK;
drop table if exists reglas_prod.regla_764;
CREATE TABLE reglas_prod.regla_764 AS
WITH predio AS (
  SELECT 
    btrim(p.id_operacion) AS id_operacion,
    btrim(p.numero_predial_nacional)::varchar(30) AS npn,
...
SELECT
  '764'::text                        AS regla,
  't_cr_UnidadConstruccion'::text      AS objeto,
  'prod.t_cr_unidadconstruccion'    AS tabla,
  NULL::int4                         AS objectid,
  NULL::varchar(38)                  AS globalid,
...
CREATE TABLE reglas_prod.regla_766 AS
WITH params AS (
...
SELECT
  ...
  'prod.t_cr_terreno'::text AS tabla,
  v.objectid                AS objectid,
  v.globalid                AS globalid,
  v.id_operacion_predio     AS id_operacion,
  v.npn,
...
CREATE TABLE reglas_prod.regla_767 AS
WITH params AS (
...
DROP TABLE IF EXISTS reglas_prod.regla_768;

CREATE TABLE reglas_prod.regla_768 AS
WITH predio AS (
  SELECT
    btrim(p.id_operacion)                         AS id_operacion,
    p.objectid,
...
DROP TABLE IF EXISTS reglas_prod.regla_769;

CREATE TABLE reglas_prod.regla_769 AS
WITH predio AS (
  SELECT
    p.objectid,
    p.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_770;

CREATE TABLE reglas_prod.regla_770 AS
WITH predio AS (
  SELECT
    p.objectid,
    p.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_793;

CREATE TABLE reglas_prod.regla_793 AS
WITH predio AS (
  SELECT 
    p.objectid,
    p.globalid,
...
avaluo AS (
  ...
  FROM prod.t_ilc_estructuraavaluo a
),
pa AS (
  SELECT
    pr.objectid   AS predio_objectid,
...
SELECT
  ...
  'prod.t_ilc_estructuraavaluo'::text AS tabla,
...
DROP TABLE IF EXISTS reglas_prod.regla_794;

CREATE TABLE reglas_prod.regla_794 AS
WITH predio AS (
  SELECT
    p.objectid,
    p.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_785;

CREATE TABLE reglas_prod.regla_785 AS
WITH predio AS (
  SELECT
    btrim(p.id_operacion)                         AS predio_id,
    btrim(p.numero_predial_nacional)::varchar(30) AS numero_predial
...
DROP TABLE IF EXISTS reglas_prod.regla_786;

CREATE TABLE reglas_prod.regla_786 AS
WITH predio AS (
  SELECT
    btrim(p.id_operacion)                         AS predio_id,
    btrim(p.numero_predial_nacional)::varchar(30) AS numero_predial
...
DROP TABLE IF EXISTS reglas_prod.regla_787;

CREATE TABLE reglas_prod.regla_787 AS
WITH predio AS (
  SELECT
    btrim(p.id_operacion)                         AS predio_id,
    btrim(p.numero_predial_nacional)::varchar(30) AS numero_predial
...
DROP TABLE IF EXISTS reglas_prod.regla_788;

CREATE TABLE reglas_prod.regla_788 AS
WITH predio AS (
...
--789
DROP TABLE IF EXISTS reglas_prod.regla_789;

CREATE TABLE reglas_prod.regla_789 AS
WITH dalc AS (
...
ea AS (
  ...
  FROM prod.t_ilc_estructuraavaluo e
)
SELECT
    '789'::text AS regla,
...
--790
DROP TABLE IF EXISTS reglas_prod.regla_790;

CREATE TABLE reglas_prod.regla_790 AS
WITH av AS (
  SELECT
    e.objectid,
    e.globalid,
...
--791
DROP TABLE IF EXISTS reglas_prod.regla_791;

CREATE TABLE reglas_prod.regla_791 AS
WITH av AS (
  SELECT
    e.objectid,
    e.globalid,
...
--765
DROP TABLE IF EXISTS reglas_prod.regla_765;

CREATE TABLE reglas_prod.regla_765 AS
...
DROP TABLE IF EXISTS reglas_prod.regla_784;

CREATE TABLE reglas_prod.regla_784 AS
WITH predio AS (
  SELECT btrim(p.id_operacion) AS id_operacion,
         btrim(p.numero_predial_nacional)::varchar(30) AS npn
  FROM prod.t_ilc_predio p
...
--792
DROP TABLE IF EXISTS reglas_prod.regla_792;

CREATE TABLE reglas_prod.regla_792 AS
WITH av AS (
  SELECT
    e.objectid,
    e.globalid,
...
DROP TABLE IF EXISTS reglas_prod.regla_795;

CREATE TABLE reglas_prod.regla_795 AS
WITH base AS (
  SELECT
    e.objectid,
    e.globalid,
...
calc AS (
  ...
  FROM base b
  LEFT JOIN prod.t_vm_zonahomogeneageoeconomicorural r
    ON r.id_zona = b.id_zona
  LEFT JOIN prod.t_vm_zonahomogeneageoeconomicaurbana u
...
--796
DROP TABLE IF EXISTS reglas_prod.regla_796;

CREATE TABLE reglas_prod.regla_796 AS
WITH params AS (
  SELECT 
...
vm AS (   -- valores unitarios por tipología
  ...
  FROM prod.t_vm_tipologiaconstruccion v
),
...
--869
DROP TABLE IF EXISTS reglas_prod.regla_869;

CREATE TABLE reglas_prod.regla_869 AS
...
--879
DROP TABLE IF EXISTS reglas_prod.regla_879;

CREATE TABLE reglas_prod.regla_879 AS
WITH enp AS (
  SELECT
    btrim(d.id_operacion_predio)                                   AS id_operacion,
    d.objectid,
...
--880
DROP TABLE IF EXISTS reglas_prod.regla_880;

CREATE TABLE reglas_prod.regla_880 AS
WITH enp AS (
  SELECT
    btrim(d.id_operacion_predio)        AS id_operacion,
    d.objectid,
...
--882
DROP TABLE IF EXISTS reglas_prod.regla_882;

CREATE TABLE reglas_prod.regla_882 AS
...
-- Regla 883 
DROP TABLE IF EXISTS reglas_prod.regla_883;

CREATE TABLE reglas_prod.regla_883 AS
...
--884
DROP TABLE IF EXISTS reglas_prod.regla_884;

CREATE TABLE reglas_prod.regla_884 AS
WITH enp AS (
  SELECT
    btrim(d.id_operacion_predio)                                    AS id_operacion,
    d.objectid,
...
-- Regla 885 
DROP TABLE IF EXISTS reglas_prod.regla_885;

CREATE TABLE reglas_prod.regla_885 AS
...
lip AS (        -- universo del listado inicial (AJUSTA si el nombre/columna difiere)
  ...
  FROM prod.listado_inicial_de_predios l
),
comp AS (
  SELECT
    e.id_operacion,
...
--886
DROP TABLE IF EXISTS reglas_prod.regla_886;

CREATE TABLE reglas_prod.regla_886 AS
WITH enp AS (
  SELECT DISTINCT
    btrim(d.id_operacion_predio)        AS id_operacion,
    UPPER(btrim(d.novedad_numero_tipo)) AS tipo_novedad
...
--682
DROP TABLE IF EXISTS reglas_prod.regla_682;

CREATE TABLE reglas_prod.regla_682 AS
WITH pred AS (
  SELECT
    p.objectid,
    p.globalid,
...
--683
DROP TABLE IF EXISTS reglas_prod.regla_683;

CREATE TABLE reglas_prod.regla_683 AS
WITH pred AS (
  SELECT
    p.objectid,
    p.globalid,
